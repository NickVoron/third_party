...
static void psd_stream_get_object_list(psd_context * context, psd_layer_record * layer);
...
// 'tdta' = RAW Data
static void psd_stream_get_raw(psd_context * context, psd_layer_record * layer)
{
	desc_pdf(context, layer);
// 	psd_int length;
// 
// 	// Length of data to follow
// 	length = psd_stream_get_int(context);
...
// 'Objc' = Descriptor
void psd_stream_get_object_descriptor(psd_context * context, psd_layer_record * layer)
{
	psd_uint type;
	...
	while(number_items --)
	{
		...
		switch(type)
		{
			...
			case 'GlbO':	// GlobalObject same as Descriptor
				...
				break;
			case 'VlLs':
				psd_stream_get_object_list(context, layer);
				break;
			case 'doub':
			...
			case 'alis':
				...
				break;
			case 'tdta':
				psd_stream_get_raw(context, layer);
				break;
			default:
				psd_assert(0);
				break;
...
void psd_stream_get_object_null(psd_uint type, psd_context * context, psd_layer_record * layer)
{
	switch(type)
	{
		case 'obj ':
	...
	for(i = 0; i < number_point; i ++)
	{
		...
		while(number_param --)
		{
			...
			switch(key)
			{
				...
				default:
					psd_assert(0);
					psd_stream_get_object_null(type, context, 0);
					break;
			}
		}
	}
