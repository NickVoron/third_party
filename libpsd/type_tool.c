...
psd_status psd_get_layer_type_tool_object(psd_context * context, psd_layer_record * layer)
{
	psd_layer_type_tool_object * data;
	psd_int i;

	layer->layer_info_type[layer->layer_info_count] = psd_layer_info_type_type_tool_object;

	data = (psd_layer_type_tool_object *) psd_malloc(sizeof(psd_layer_type_tool_object));
	if (data == NULL)
		return psd_status_malloc_failed;
	memset(data, 0, sizeof(psd_layer_type_tool_object));
	layer->layer_info_data[layer->layer_info_count] = (psd_uint) data;
	layer->layer_info_count++;
	...
	psd_assert(psd_stream_get_int(context) == 16);
 	...
 	psd_stream_get_object_descriptor(context, layer);
	...
	psd_assert(psd_stream_get_int(context) == 16);
   	...
   	psd_stream_get_object_descriptor(context, layer);
//    
  	data->left = psd_stream_get_double(context);
   	data->top = psd_stream_get_double(context);
//     data->right = psd_stream_get_double(context);
//     data->bottom = psd_stream_get_double(context);
