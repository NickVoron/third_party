...
extern psd_status psd_get_layer_type_tool_object(psd_context * context, psd_layer_record * layer);
...
static psd_status psd_get_layer_info(psd_context * context)
{
		...
		while(context->stream.current_pos - extra_stream_pos < extra_length)
		{
			...
			switch(tag)
			{
				...
				case 'TySh':
					...
					status = psd_get_layer_type_tool_object(context, layer);
					break;
				case 'SoCo':
	...
	{		
		...
		layer->group_layer = group >= 0 ? groups[group] : 0;
