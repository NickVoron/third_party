...
//			assert(padLen > 0 && padLen <= 16);
			mem::memcpy(block, input, 16 - padLen);
			mem::memset(block + 16 - padLen, padLen, padLen);
			encrypt(block,outBuffer);
		break;
		case CBC:
			iv = m_initVector;
...
#if STRICT_ALIGN
				mem::memcpy(iv, input, 16);
				mem::memcpy(outBuf, block, 16);
#else
...
int Rijndael::padDecrypt(const UINT8 *input, int inputOctets, UINT8 *outBuffer)
{
	...
	switch(m_mode){
		case ECB:
			...
			mem::memcpy(outBuffer, block, 16 - padLen);
		break;	
		case CBC:
			mem::memcpy(iv, m_initVector, 16);
			/* all blocks but last */
